...
export default function VerificationSuccess() {
  ...
  const normalizedRole = String(savedRole || savedUser?.role || savedUser?.accountType || savedUser?.userType || "purchaser").toLowerCase();
  const dashboardPath = normalizedRole.includes("creator") ? "/creator-dashboard" : "/dashboard";
      ...
      <div className="md:grid md:grid-cols-2 pt-25 md:pt-0 min-h-screen">
        ...
        <div className="min-h-screen flex flex-col md:justify-center items-center p-10 sm:p-12 md:p-16 py-10 gap-8 mx-auto max-w-md md:max-w-none md:w-auto border-[14px] border-[#2EC5BC] md:border-none">
          ...
          <div className="flex flex-col items-center text-center px-10 sm:px-16">
            ...
            <h4 className="text-base/7 text-[#4A5568] font-[400]">
              Your email has been verified. You can now access your account and start exploring our educational resources.
            </h4>
          </div>
          <Link
            to={dashboardPath}
            className="rounded-full flex items-center justify-center bg-[#2EC5BC] px-10 sm:px-20 py-3 mb-4 font-semibold text-white cursor-pointer hover:opacity-[0.85]"
          >
            Go to Dashboard
          </Link>
        </div>
